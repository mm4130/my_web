...
3⃣ رفع مشکلات شما در ربات پشتیبان پيامرسان ساز 🗣
4⃣ دستور /creator که نشان دهنده ساخته شدن ربات شما توسط @pmr_creator_bot است از ربات شما حذف میشود. 😉
5⃣ در صورت اضافه شدن امکانات جدید به ربات پی وی رسان ربات های VIP اولویت اول را دارند. 🤖
6   سرعت روبات هاي VIP از روبات هاي معمولي بيشتر است!!
...
Copy Right 2017 ©
	...
	var_dump(makereq('sendMessage',[
        	'chat_id'=>$update->message->chat->id,
        	'text'=>"پروفایل خود را تنظيم کنید.",
		'parse_mode'=>'MarkDown',
        	'reply_markup'=>json_encode([
            	'keyboard'=>[
                [
			...
			$protxt = "";
			if ($name == '' && $age == '' && $bio == '') {
				$protxt = "اطلاعاتي دربارهي پروفايل در حافظه موجود نيست..!📕 ";
			}
			if ($name != '') {
				$protxt = $protxt."\nName : ".$name;
			}
